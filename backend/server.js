...
io.use(async(socket, next)=> {
    try {
        ...
        // 1. Tìm token ở mọi ngóc ngách: Auth (code), Query (URL), và Headers (Postman)
        let token = 
            socket.handshake.auth?.token || 
            socket.handshake.query?.token || 
            socket.handshake.headers?.token; // <--- Cứu tinh của bạn ở đây

        // Log ra để bạn yên tâm
        console.log("Token từ Headers:", socket.handshake.headers?.token ? "CÓ" : "KHÔNG");

        if(!token) {
            console.error("❌ Không tìm thấy token ở đâu cả!");
            return next(new Error("Xác thực thất bại: Thiếu Token"));
        }
        ...
        // Gán user vào socket để dùng sau này
        socket.user = decoded;
...
// Khởi động các logic socket (chat, join room...)
socketManager(io);

// Lắng nghe lỗi server
server.on('error', (error) => {
    console.error('Server Error:', error.message);
});
